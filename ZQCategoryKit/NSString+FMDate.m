...
+ (NSInteger)fm_compareDateWithTheLocalDate:(NSString*)aDate {
    ...
    NSDate *dateObj = [dateformater dateFromString:aDate];
...
+(int)zq_getAgeFormBirthDay:(NSString *)birtStr{
    NSDateFormatter *formatter = [[NSDateFormatter alloc] init];

    if([birtStr containsString:@"/"])
        formatter.dateFormat = @"yyyy/MM/dd";
    else
        formatter.dateFormat = @"yyyy-MM-dd";
    
    NSDate *birthDate = [formatter dateFromString:birtStr];

    NSTimeInterval dateDifference = [birthDate timeIntervalSinceNow];

    int age1 = trunc(dateDifference/(60*60*24))/365;
    
    return abs(age1);
}

+(NSString *)calculateConstellationWithBirthDay:(NSString *)birtStr{
    ...
    NSDate *date = [formatter dateFromString:birtStr];
    ...
    /**
     方法二： NSCalendar 和 NSDateComponents
